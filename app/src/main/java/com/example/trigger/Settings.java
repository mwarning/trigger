...
public class Settings {
    ...
    private static void upgradeDB() {
        ...
        if (Arrays.asList("1.9.2", "2.0.0", "2.0.1", "2.0.2", "2.0.3", "2.0.4", "2.0.5", "2.0.6", "2.1.0", "2.1.1").contains(db_version)) {
            Log.i("Settings", "Update database format from " + db_version + " to 2.1.1");
            // nothing to change
            setups = new ArrayList();
            sharedPreferences.edit().putString("db_version", "2.2.0").commit();
            db_version = "2.2.0";
        }
    }
